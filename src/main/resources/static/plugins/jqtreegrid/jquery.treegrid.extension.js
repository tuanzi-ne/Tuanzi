(function($) {
    ...
    $.fn.bootstrapTreeTable = function(options, param) {
        ...
        var _main_div = $("<div class='bootstrap-table'><div class='fixed-table-container' style='padding-bottom: 0px;'><div class='fixed-table-body'></div></div></div>");
        target.before(_main_div);
        _main_div.find(".fixed-table-body").append(target);
        ...
        if(options.toolbar){
            ...
            _tool_left_div.append($(options.toolbar).html());
        ...
        target.renderRow = function(tr,item){
            $.each(options.columns, function(index, column) {
                ...
                if(index==0&&column.field=='selectItem'){
                    hasSelectItem = true;
                    var td = $('<td style="text-align:center;width:35px"></td>');
                    if(column.radio){
                    ...
                    var td = $('<td style="text-align:center;word-break:keep-all;white-space:nowrap;'+((column.width)?('width:'+column.width):'')+'"></td>');
                    // 增加formatter渲染
                    if (column.formatter) {
                        td.html(column.formatter.call(this, item, index));
                    } else {
                        td.text(item[column.field] == null ? "":$.trim(item[column.field]));
                    }
                    tr.append(td);
                }
            });
        ...
        target.load = function(parms){
            ...
            $.each(options.columns, function(i, item) {
                ...
                if(i==0&&item.field=='selectItem'){
                    hasSelectItem = true;
                    th = $('<th style="text-align:center;width:35px;vertical-align:middle"><input name="select_item_all" type="checkbox"></input></th>');
                }else{
                    th = $('<th style="text-align:center;word-break:keep-all;white-space:nowrap;padding:8px;'+((item.width)?('width:'+item.width):'')+'"></th>');
                }
                th.text(item.title);
                thr.append(th);
            });
